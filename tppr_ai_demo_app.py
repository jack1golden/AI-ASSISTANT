    ...
    with st.sidebar:
        ...
        for rk in DETECTORS.keys():
            if st.button(f"Trigger Danger: {ROOM_NAMES[rk]}"):
                ...
                st.toast(f"Forced danger for {ROOM_NAMES[rk]}")
...
def render_facility():
    ...
    with col_map:
        st.image("assets/facility.svg", use_column_width=True)
        fig = go.Figure()
        fig.update_layout(
        ...
        for rk, d in DETECTORS.items():
            ...
            stt = gas_state(rk, ppm if ppm is not None else 0)
            update_timers(rk, stt, ts if ts is not None else datetime.utcnow())
...
def render_room():
    ...
    if rk is None or rk not in ROOM_NAMES:
        st.session_state.view = "facility"; st.rerun()
    ...
    with col_map:
        ...
        st.image(room_svg_path, use_column_width=True)
    ...
    with col_ai:
        ...
        if stt == DANGER:
            st.error("Danger detected. Advise immediate evacuation along nearest safe route. Isolate energy sources and stop process feeds if possible.")
        elif stt == WARN:
            st.warning("Warning. Levels are trending upward. Increase ventilation and prepare for evacuation if rise continues.")
        else:
