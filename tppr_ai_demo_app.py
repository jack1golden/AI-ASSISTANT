...
ROOMS = {
    "reactor": {"name": "Reactor Suite","center": (15, 10, 0),"size": (18, 12, 6),
        "detectors": [{"key": "rxr_h2s","model": "Honeywell XNX","gas": "H₂S","warn": 5.0,"danger": 10.0,"oxygen_mode": False,"room_pos": (6, 4)}],},
    "granulation": {"name": "Granulation","center": (45, 10, 0),"size": (16, 12, 5),
        "detectors": [{"key": "gran_co","model": "Sensepoint","gas": "CO","warn": 30.0,"danger": 40.0,"oxygen_mode": False,"room_pos": (5, 5)}],},
    "cleanroom": {"name": "Cleanroom (Grade C)","center": (15, 28, 0),"size": (20, 12, 4),
        "detectors": [{"key": "cr_o2","model": "Sensepoint","gas": "O₂","warn": 18.0,"danger": 17.0,"oxygen_mode": True,"room_pos": (8, 5)}],},
    "packaging": {"name": "Packaging Hall","center": (45, 28, 0),"size": (22, 12, 5),
        "detectors": [{"key": "pack_ch4","model": "XNX","gas": "CH₄","warn": 45.0,"danger": 55.0,"oxygen_mode": False,"room_pos": (9, 4)}],},
    "warehouse": {"name": "Warehouse (API)","center": (75, 10, 0),"size": (18, 12, 6),
        "detectors": [{"key": "wh_h2","model": "Searchline","gas": "H₂","warn": 35.0,"danger": 50.0,"oxygen_mode": False,"room_pos": (6, 4)}],},
    "utilities": {"name": "Utilities / Boilers","center": (75, 28, 0),"size": (18, 12, 6),
        "detectors": [{"key": "util_ch4","model": "XNX","gas": "CH₄","warn": 45.0,"danger": 55.0,"oxygen_mode": False,"room_pos": (6, 4)}],},
}
...
for key, default in [
    ("view","facility3d"), ("room_key",None), ("detector_key",None),
    ("demo_index",0), ("incident_log",[]), ("free_play",False),
    ("show_route",False), ("last_event",None),
]:
...
def update_timers(detector_key: str, state: str, ts: datetime):
    ...
    if state != t["state"]:
        ...
        st.session_state.incident_log.append({"timestamp": ts,"room": ROOMS[room_key]["name"],"detector": d["model"],"from": t["state"],"to": state,})
    if state == DANGER:
...
def prediction_curve(room_key: str, horizon: int = 15) -> pd.DataFrame:
    ...
    recent = df.tail(5)["ppm"].values; deltas = np.diff(recent); slope = deltas.mean() if len(deltas) else 0.0
...
def df_table(df: pd.DataFrame): return st.dataframe(df, use_container_width=True, hide_index=True)

# ---- 3D Facility ----
def _room_mesh(room_key: str):
    cx, cy, cz = ROOMS[room_key]["center"]; sx, sy, sz = ROOMS[room_key]["size"]
    x0, x1 = cx - sx/2, cx + sx/2; y0, y1 = cy - sy/2, cy + sy/2; z0, z1 = cz, cz + sz
...
def worst_state_for_room(room_key: str):
    ppm, _ = last_ppm(room_key); worst = SAFE
...
def build_facility_3d(show_route=False, start_room=None):
    fig = go.Figure()
    fig.add_trace(go.Scatter3d(x=[0, 90, 90, 0, 0], y=[0, 0, 38, 38, 0], z=[0,0,0,0,0], mode="lines", line=dict(width=4), hoverinfo="skip"))
    centers_x, centers_y, centers_z, labels, colors = [], [], [], [], []
    for rk in ROOMS.keys():
        X, Y, Z, I, J, K = _room_mesh(rk); state = worst_state_for_room(rk)
        fig.add_trace(go.Mesh3d(x=X, y=Y, z=Z, i=I, j=J, k=K, color=STATE_COLORS[state], opacity=0.35, hoverinfo="skip"))
        cx, cy, cz = ROOMS[rk]["center"]; centers_x.append(cx); centers_y.append(cy); centers_z.append(cz + ROOMS[rk]["size"][2] + 1.2)
        labels.append(ROOMS[rk]["name"]); colors.append(STATE_COLORS[state])
    fig.add_trace(go.Scatter3d(x=centers_x, y=centers_y, z=centers_z, mode="markers+text", marker=dict(size=8, color=colors),
                               text=labels, textposition="top center", hovertemplate="%{text}<extra></extra>"))
    for name, (ex,ey,ez) in EXITS.items():
        fig.add_trace(go.Scatter3d(x=[ex], y=[ey], z=[ez], mode="markers+text", marker=dict(size=5, color="#10b981", symbol="square"),
                                   text=[name.title()+" Exit"], textposition="bottom center", hoverinfo="skip"))
    ...
    fig.update_layout(height=640, margin=dict(l=0,r=0,t=0,b=0),
                      scene=dict(xaxis=dict(visible=False), yaxis=dict(visible=False), zaxis=dict(visible=False, range=[0, 12]),
...
def compute_route_3d(start_room_key: str):
    cx, cy, cz = ROOMS[start_room_key]["center"]; east = EXITS["east"]; west = EXITS["west"]
    target = east if abs(east[0]-cx) < abs(cx-west[0]) else west; mid = (target[0], cy, 0)
...
def get_clicked_room_from_payload(payload: dict):
    ...
    for rk, r in ROOMS.items():
        cx, cy, cz = r["center"]; d = (cx-x)**2 + (cy-y)**2 + (cz-0)**2
        if d < bestd: bestd, best = d, rk
    return best

# ---- Define apply_simulation BEFORE any potential usage ----
def apply_simulation(room_key: str, mode: str, intensity: int, duration: int):
    df = st.session_state.data.copy(); idx = st.session_state.demo_index
    if mode == "Spike":
        mask = (df["room"] == room_key) & (df.index >= idx) & (df.index < idx + duration); df.loc[mask, "ppm"] = df.loc[mask, "ppm"] + float(intensity)
    elif mode == "Ramp":
        for i in range(duration):
            mask = (df["room"] == room_key) & (df.index == idx + i); df.loc[mask, "ppm"] = df.loc[mask, "ppm"] + float(intensity) * (i + 1) / duration
    elif mode == "O₂ drop":
        for i in range(duration):
            mask = (df["room"] == room_key) & (df.index == idx + i); df.loc[mask, "ppm"] = df.loc[mask, "ppm"] - float(intensity) * (i + 1) / duration
    elif mode == "CO spike":
        for i in range(duration):
            mask = (df["room"] == room_key) & (df.index == idx + i); df.loc[mask, "ppm"] = df.loc[mask, "ppm"] + float(intensity) * 0.4
...
# ---- Dev sidebar (uses apply_simulation already-defined) ----
...
with st.expander("Debug", expanded=False):
    st.write({"view": st.session_state.get("view"),"room_key": st.session_state.get("room_key"),
              "detector_key": st.session_state.get("detector_key"),"demo_index": st.session_state.get("demo_index"),
              "free_play": st.session_state.get("free_play"),"show_route": st.session_state.get("show_route"),})
...
def render_facility_3d():
    render_breadcrumbs(); st.markdown("#### Facility — 3D Overview")
    ...
    with col_map:
        ...
        if clicked:
            payload = clicked[0]; st.session_state.last_event = payload
            rk = get_clicked_room_from_payload(payload)
            if rk: st.session_state.room_key = rk; st.session_state.detector_key = None; st.session_state.view = "room"
    with col_side:
        ...
        df_lead = pd.DataFrame(rows).sort_values("Longest Danger (s)", ascending=False); df_table(df_lead)

def render_room():
    render_breadcrumbs(); rk = st.session_state.room_key
    if rk is None or rk not in ROOMS: st.session_state.view = "facility3d"; return
    room = ROOMS[rk]; st.markdown(f"### {room['name']}")
    ...
    with col_map:
        ...
        for det in room["detectors"]:
            stt = gas_state(det, ppm or 0); dx, dy = det["room_pos"]
            xs.append(dx); ys.append(dy); labels.append(f"{det['model']} ({det['gas']})"); colors.append(STATE_COLORS[stt])
        fig.add_trace(go.Scatter(x=xs, y=ys, mode="markers+text", marker=dict(size=20, color=colors, line=dict(color="#0b0b0b", width=1)),
                                 text=[l.split(" (")[0] for l in labels], textposition="top center", hovertemplate="%{text}<extra></extra>", showlegend=False))
...
def render_detector():
    render_breadcrumbs(); dk = st.session_state.detector_key
    ...
    ppm, ts = last_ppm(rk); stt = gas_state(det, ppm or 0)
    danger_time = time_in_state_str(dk, DANGER, ts if ts is not None else datetime.utcnow()); warn_time = time_in_state_str(dk, WARN, ts if ts is not None else datetime.utcnow())
    c1,c2,c3,c4 = st.columns(4)
    c1.metric("Live Reading", f"{ppm:.1f} ppm" if ppm is not None else "—"); c2.metric("State", stt.upper()); c3.metric("Time in Danger", danger_time); c4.metric("Time in Warning", warn_time)
    ...
    if not df_room_full.empty:
        df_room_full["roll_mean"] = df_room_full["ppm"].rolling(12, min_periods=3).mean(); df_room_full["roll_std"] = df_room_full["ppm"].rolling(12, min_periods=3).std().replace(0, np.nan)
        ...
        c1.metric("Anomaly (z-score)", f"{z:.2f}"); c2.metric("Slope (Δppm/tick)", f"{slope:.2f}")
        ...
        with st.expander("What‑if: Ventilation & Mitigation"):
            ...
            adj_slope = slope * (1 - reduction/100.0); horizon = 30; start_ppm = last_val
            future = [max(0.0, start_ppm + adj_slope * i) for i in range(1, horizon+1)]
            risk = "LOW"; projected = future[9] if len(future) >= 10 else future[-1]
            if det.get("oxygen_mode"):
    ...
    if det.get("oxygen_mode"):
        fig.add_hrect(y0=-1e6, y1=danger, fillcolor=STATE_COLORS[DANGER], opacity=0.10, line_width=0); fig.add_hrect(y0=danger, y1=warn, fillcolor=STATE_COLORS[WARN], opacity=0.08, line_width=0)
    else:
        fig.add_hrect(y0=danger, y1=1e6, fillcolor=STATE_COLORS[DANGER], opacity=0.10, line_width=0); fig.add_hrect(y0=warn, y1=danger, fillcolor=STATE_COLORS[WARN], opacity=0.08, line_width=0)
    fig.update_layout(template="plotly_dark", height=420, margin=dict(l=10, r=10, t=10, b=10), legend=dict(orientation="h")); st.plotly_chart(fig, use_container_width=True)
    c1,c2,_ = st.columns([1,1,2])
    if c1.button("⬅️ Room"): st.session_state.view = "room"
    if c2.button("Evacuation"): st.session_state.view = "evac"

def render_evac():
    st.markdown("### Evacuation Mode"); st.info("3D route to safe exit. Follow green labels.")
    fig = build_facility_3d(show_route=True, start_room=st.session_state.room_key or "reactor"); st.plotly_chart(fig, use_container_width=True)
    c1,c2,_ = st.columns([1,1,2])
...
for room_key, room in ROOMS.items():
    ppm, ts = last_ppm(room_key); ts_use = ts if ts is not None else now_ts
    for det in room["detectors"]:
        stt = gas_state(det, ppm if ppm is not None else 0); update_timers(det["key"], stt, ts_use)
...
if view == "facility3d": render_breadcrumbs(); render_facility_3d()
elif view == "room": render_breadcrumbs(); render_room()
elif view == "detector": render_breadcrumbs(); render_detector()
elif view == "evac": render_breadcrumbs(); render_evac()
else: render_breadcrumbs(); render_facility_3d()
